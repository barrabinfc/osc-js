  ...
  createClass(BridgePlugin, [{
    ...
    value: function open() {
      ...
      this.socket.bind({
        ...
        var wsopt = { host: options.wsServer.host, port: options.wsServer.port };
        if (options.wsServer.server) {
          _this2.websocket = new WebsocketServer({ server: options.wsServer.server });
        } else {
          _this2.websocket = new WebsocketServer(wsopt);
        }
        ...
        _this2.websocket.on('connection', function (client) {
          client.on('message', function (message) {
            _this2.send(message, { receiver: 'udp' });
...
var WebsocketServerPlugin = function () {
  ...
  createClass(WebsocketServerPlugin, [{
    ...
    value: function open() {
      ...
      if (server) {
        this.socket = new WebsocketServer$1({ server: server });
      } else {
    ...
    value: function close() {
      ...
      this.socket.close(function () {
        _this2.socketStatus = STATUS$4.IS_CLOSED;
        _this2.notify('close');
      });
    }
  }, {
...
var OSC$1 = function (_OSCBase) {
  ...
  function OSC$$1(options) {
    ...
    return possibleConstructorReturn(this, (OSC$$1.__proto__ || Object.getPrototypeOf(OSC$$1)).call(this, Object.assign({}, defaultOptions$6, options)));
  }
  return OSC$$1;
}(OSC);
OSC$1.DatagramPlugin = DatagramPlugin;
OSC$1.WebsocketClientPlugin = WebsocketClientPlugin;
OSC$1.WebsocketServerPlugin = WebsocketServerPlugin;
OSC$1.BridgePlugin = BridgePlugin;
